...
def create_chain_template_request(version, auth_headers, data = {})
  post "/api/chain_templates", data, {'Content-Type' => "application/json", 'Accept' => "application/vnd.ink.#{version}" }.merge(auth_headers)
end

def update_chain_template_request(version, auth_headers, data = {})
  patch "/api/chain_templates/#{data[:id]}", data.to_json, {'Content-Type' => "application/json", 'Accept' => "application/vnd.ink.#{version}" }.merge(auth_headers)
end
...
def execute_chain_template_request(version, auth_headers, data = {})
  id = data[:id]
  data.delete(:id)
